...
output$plotlyplot <- renderPlotly(
 fics_disponibles2() %>% 
   filter(cod == "5_16_1_10936" & Particip. == "501") %>% 
   select(1,12) %>%
   drop_na() %>% 
   plot_time_series(.date_var = Fecha, .value = !!rlang::sym(names(fics_disponibles2())[12]), .smooth = FALSE)
  )
...
output$plotlyplot2 <- renderPlotly(
 fics_disponibles2() %>% 
   ...
   select(1,14) %>% 
   drop_na() %>% 
   plot_time_series(.date_var = Fecha, .value = !!rlang::sym(names(fics_disponibles2())[14]), .smooth = FALSE)
  )


output$plotlyplot3 <- renderPlotly({
  
  ptl <- reactive(fics_disponibles2() %>% 
                    filter((cod == "5_16_1_10936" & Particip. == "501")|(cod == "5_16_1_18462" & Particip. == "513")) %>% 
                    drop_na() %>% 
                    ggplot(aes(x = Fecha, y = !!rlang::sym(names(fics_disponibles2())[14]), colour = cod)) +
                    geom_line() +
                    ...
                    scale_color_manual(values= c("#708090", "#8B0000", "#2F4F4F", "#DAA520", "#483D8B", "#B8860B", "#556B2F", "#FFA500", "#8B4513", "#1E90FF"))
                  )
